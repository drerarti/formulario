(function () {
  ...
  function applyStaticCopy() {
    if (refs.reservaSearch) refs.reservaSearch.placeholder = "Buscar por cliente, agente, unidad o codigo";
  ...
  function createReservationCard(item) {
    ...
    if (item.puede_validar) {
      actions.appendChild(AppCore.createElement("button", { className: "btn-outline", text: "Validar", attrs: { type: "button" }, events: { click: () => window.validar && window.validar(item.id) } }));
    }

    if (item.puede_rechazar) {
      actions.appendChild(AppCore.createElement("button", { className: "btn-danger", text: "Rechazar", attrs: { type: "button" }, events: { click: () => window.rechazar && window.rechazar(item.id, item.unidad_record_id || item.unidad) } }));
    }

    if (item.puede_negociar) {
      actions.appendChild(AppCore.createElement("button", { className: "btn-outline", text: "Negociar", attrs: { type: "button" }, events: { click: () => window.mostrarNegociacion && window.mostrarNegociacion(item.id) } }));
      if (!item.boleta_emitida) {
        actions.appendChild(AppCore.createElement("button", { className: "btn-outline", text: "Boleta", attrs: { type: "button" }, events: { click: () => { window.location.href = `/preview-boleta.html?id=${encodeURIComponent(item.id)}`; } } }));
      }
    }
  ...
  function createSaleCard(item) {
    const card = AppCore.createElement("article", { className: "sale-card-admin" }, [
      ...
      AppCore.createElement("div", { className: "collection-stats-grid" }, [
        ...
        createMetaPill("Proxima cuota", item.proxima_cuota ? formatDate(item.proxima_cuota) : "Sin cronograma", item.cuotas_morosas > 0 ? "rejected" : item.cuotas_vencidas > 0 ? "pending" : "neutral")
      ]),
      ...
      card.appendChild(AppCore.createElement("div", {
        ...
        AppCore.createElement("span", {
          text: item.cuotas_morosas > 0
            ? `${item.cuotas_morosas} cuota(s) en mora requieren atencion inmediata.`
  ...
  function createUnitCard(item) {
    ...
    const select = AppCore.createElement("select", { attrs: { id: `estado-${item.id}` } });
    ["Disponible", "Reservado", "Vendido"].forEach((value) => {
      select.appendChild(AppCore.createElement("option", {
        text: value,
        attrs: { value, selected: value === item.estado ? "selected" : null }
      }));
    });
    card.appendChild(select);

    const button = AppCore.createElement("button", { className: "btn-primary", text: "Guardar cambios", attrs: { type: "button" } });
    button.addEventListener("click", () => window.guardarUnidad && window.guardarUnidad(item.id));
  ...
  function renderSalesChart() {
    ...
    state.chart = new window.Chart(refs.chartCanvas.getContext("2d"), {
      type: "line",
      data: {
        labels,
        datasets: [
  ...
  function renderReservations() {
    ...
    buildPagination(refs.reservasPagination, paginated.page, paginated.totalPages, (page) => { state.reservationsPage = page; renderReservations(); });
  }
  ...
  function renderSales() {
    ...
    buildPagination(refs.ventasPagination, paginated.page, paginated.totalPages, (page) => { state.salesPage = page; renderSales(); });
  }
  ...
  window.guardarNegociacion = async (id) => {
    try {
      ...
      showAdminAlert("Negociacion guardada correctamente.");
    } catch (error) {
      showAdminAlert(AppCore.getErrorMessage(error, "No se pudo guardar la negociacion."));
    }
  };
  ...
  async function init() {
    if (init.done) return;
    init.done = true;
    if (refs.headerTitle) refs.headerTitle.textContent = "Panel de Administracion";
    if (refs.subtitle) refs.subtitle.textContent = "Operacion comercial, control y seguimiento.";
    document.title = "Ayllu Laguna Huaypo | Administracion";
    applyStaticCopy();
    bindEvents();
    await refreshAll();
